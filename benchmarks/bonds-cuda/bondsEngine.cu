...
void runBoundsEngine(const int repeat, const int num_bond, const int bond_start, const int bond_end, const int multiplier)
{
  //can run multiple times with different number of bonds by uncommenting these lines
  int nBondsArray[] = {num_bond};
  ...
  for (int numTime=0; numTime < 1; numTime++)
  {
    ...
    for (numBond = 0; numBond < numBonds; numBond++)
    {
      ...
      bond.rate = 0.08 + ((float)rand()/(float)RAND_MAX - 0.5)*35;
    ...
    numBonds = bond_end - bond_start;
    std::cout << "Num bonds: " << numBonds << std::endl;
    inArgsHost.discountCurve = inArgsHost.discountCurve + bond_start;
    inArgsHost.repoCurve = inArgsHost.repoCurve + bond_start;
    inArgsHost.currDate = inArgsHost.currDate + bond_start;
    inArgsHost.maturityDate = inArgsHost.maturityDate + bond_start;
    inArgsHost.bondCleanPrice = inArgsHost.bondCleanPrice + bond_start;
    inArgsHost.bond = inArgsHost.bond + bond_start;
    inArgsHost.dummyStrike = inArgsHost.dummyStrike + bond_start;
    ...
    gettimeofday(&start, NULL);
      ...
      ktimeGpu += getBondsResultsGpu(inArgsHost, resultsFromGpu, numBonds, repeat, multiplier);
...
{
  if (argc != 6) {
    printf("Usage: %s <repeat> <num_bonds> <bond_start> <bond_end> <multiplier>\n", argv[0]);
    return 1;
  }
  const int repeat = atoi(argv[1]);
  const int num_bonds = atoi(argv[2]);
  const int bond_start = atoi(argv[3]);
  const int bond_end = atoi(argv[4]);
  const int multiplier = atoi(argv[5]);
  runBoundsEngine(repeat, num_bonds, bond_start, bond_end, multiplier);
}
