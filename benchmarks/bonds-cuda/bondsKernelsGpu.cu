...
void write_results_to_file(std::string filename, size_t num_bonds, bondStructSoA inputs, dataType* accrued_amt)
{
  std::ofstream results_file;
  // open file for writing binary
  results_file.open(filename.c_str(), std::ios::out | std::ios::binary);
  // write the number of bonds
  results_file.write((char*)&num_bonds, sizeof(size_t));

  for(int cur_bond = 0; cur_bond < num_bonds; cur_bond++) {
    dataType startDate_f[4];
    dataType maturityDate_f[4];
    for(int i = 0; i < 4; i++) {
      int start = inputs.startDate[4*cur_bond+i];
      startDate_f[i] = (dataType)start;

      int maturity = inputs.maturityDate[4*cur_bond+i];
      maturityDate_f[i] = (dataType)maturity;
    }
    dataType rate = inputs.rate[cur_bond];

    // write rate, then start dates and maturity dates as float to file
    results_file.write((char*)&rate, sizeof(dataType));
    results_file.write((char*)&startDate_f, sizeof(dataType)*4);
    results_file.write((char*)&maturityDate_f, sizeof(dataType)*4);
    results_file.write((char*)&accrued_amt[cur_bond], sizeof(dataType));
  }
...
long getBondsResultsGpu(inArgsStruct inArgsHost, resultsStruct resultsFromGpu, int numBonds, int repeat, int multiplier)
{
  ...
  bondStruct* bondGpu;
  bondStructSoA bondGpuSoa;
  bondStructSoA bondCpuSoa;
  ...
  cudaMalloc((void**)&bondGpuSoa.startDate, numBonds*sizeof(bondsDateStruct));
  cudaMalloc((void**)&bondGpuSoa.maturityDate, numBonds*sizeof(bondsDateStruct));
  cudaMalloc((void**)&bondGpuSoa.rate, numBonds*sizeof(dataType));
  ...
  int *startDateSoa = new int[4*numBonds];
  int *maturityDateSoa = new int[4*numBonds];
  dataType *rateSoa = new dataType[numBonds];
  for(int i = 0 ; i < numBonds; i++) {
    startDateSoa[i*4+0] = inArgsHost.bond[i].startDate.month;
    startDateSoa[i*4+1] = inArgsHost.bond[i].startDate.day;
    startDateSoa[i*4+2] = inArgsHost.bond[i].startDate.year;
    startDateSoa[i*4+3] = inArgsHost.bond[i].startDate.dateSerialNum;

    maturityDateSoa[i*4+0] = inArgsHost.bond[i].maturityDate.month;
    maturityDateSoa[i*4+1] = inArgsHost.bond[i].maturityDate.day;
    maturityDateSoa[i*4+2] = inArgsHost.bond[i].maturityDate.year;
    maturityDateSoa[i*4+3] = inArgsHost.bond[i].maturityDate.dateSerialNum;

    rateSoa[i] = inArgsHost.bond[i].rate;
  }

  bondCpuSoa.startDate = startDateSoa;
  bondCpuSoa.maturityDate = maturityDateSoa;
  bondCpuSoa.rate = rateSoa;
  ...
  HtoD.recordStart();
  cudaMemcpy(bondGpuSoa.startDate, startDateSoa, 4*numBonds*sizeof(int), cudaMemcpyHostToDevice);
  cudaMemcpy(bondGpuSoa.maturityDate, maturityDateSoa, 4*numBonds*sizeof(int), cudaMemcpyHostToDevice);
  cudaMemcpy(bondGpuSoa.rate, rateSoa, numBonds*sizeof(dataType), cudaMemcpyHostToDevice);
  ...
  #ifdef APPROX
  auto bondgpusoarate = bondGpuSoa.rate;
  auto bondgpusoastartDate = bondGpuSoa.startDate;
  auto bondgpusoamaturityDate = bondGpuSoa.maturityDate;

  int ni = numBonds/multiplier;
  #pragma approx declare tensor_functor(bond_functor: [i,0:9] = ([i*multiplier:i*multiplier+multiplier], [i*multiplier*4:i*multiplier*4+(multiplier*4)], [i*4*multiplier:i*4*multiplier+(4*multiplier)]))
  #pragma approx declare tensor_functor(opt_functor: [i, 0:1] = ([i*multiplier:i*multiplier+multiplier]))

  #pragma approx declare tensor(bond: bond_functor(bondgpusoarate[0:ni], bondgpusoastartDate[0:ni], bondgpusoamaturityDate[0:ni]))
  #pragma approx ml(infer) in(bond) out(opt_functor(accruedAmountCurrDateGpu[0:ni])) label("BondsKernel")
  #endif
  {
    Kernel.recordStart();
  ...
  std::cout << "GPU Execution time is " << ((float) ktime)/(1000*repeat) << " msec" << std::endl;

  // write_results_to_file("results.bin", numBonds, bondCpuSoa, resultsFromGpu.accruedAmountCurrDate);
  #if CAPTURE_OUTPUT
  int a = 0;
  int *b = &a;
  #pragma approx declare tensor_functor(dmy: [i] = ([i]))
  #pragma approx declare tensor(dmy_t: dmy(b[0:1]))
  #pragma approx declare tensor_functor(opt_functor2: [i, 0:1] = ([i]))
  #pragma approx ml(offline) in(dmy_t) out(opt_functor2(accruedAmountCurrDateGpu[0:numBonds])) label("BondsOutput")
  {
  ...
  free(startDateSoa);
  free(maturityDateSoa);
  free(rateSoa);
  cudaFree(bondGpuSoa.startDate);
  cudaFree(bondGpuSoa.maturityDate);
  cudaFree(bondGpuSoa.rate);
