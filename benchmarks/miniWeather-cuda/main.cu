...
int main(int argc, char **argv) {
  ...
  int state_size = (nz+2*hs)*(nx+2*hs)*NUM_VARS;
  int state_size_byte = (nz+2*hs)*(nx+2*hs)*NUM_VARS*sizeof(double);

  double *state_final = nullptr;
  approx::util::warmup();
  ...
  double *d_state_tmp;
  state_size = (nz+2*hs)*(nx+2*hs)*NUM_VARS;
  state_size_byte = (nz+2*hs)*(nx+2*hs)*NUM_VARS*sizeof(double);
  if(!state_final) {
      state_final = (double*) malloc(state_size_byte);
  }
  ...
  // #pragma approx declare tensor_functor(state_functor: [i, j, 0:4] = [i,j], [i,j], [i,j], [i,j])

  // in the final, we will add transposition here, for now we'll just do the straignt mapping and
  // transpose in the other.
  #ifdef APPROX
  #pragma approx declare tensor_functor(state_functor: [i, j, k] = ([i,j,k]))
  #pragma approx declare tensor_functor(state_functor2: [i, j, k] = ([i,j, k]))
  #endif
  std::cout << "NX is " << nx << "\n";
  std::cout << "NZ is " << nz << "\n";
  ...
  while (etime < sim_time) {
    ...
    #pragma approx ml(infer) in(ipt_tens) out(state_functor2(d_state[0:4, 0:nz+(2*hs), 0:nx+(2*hs)])) label("MiniWeatherTimestep")
    #endif
    {
  ...
  memcpy(state_final, state, state_size_byte);
  finalize();
  cudaFree(d_state);
  ...
  int *b = &a;
  #pragma approx declare tensor_functor(state_functor: [i, j, k] = ([i,j,k]))
  #pragma approx declare tensor_functor(i_pt: [i] = ([i]))
  #pragma approx declare tensor(ten: i_pt(b[0:1]))
  #pragma approx ml(offline) in(ten) out(state_functor(state_final[0:4, 0:nz+(2*hs), 0:nx+(2*hs)])) label("MiniWeatherOutput")
  {

  }
  #else
  #warning "Not capturing the output"
  #endif
  return 0;
}
