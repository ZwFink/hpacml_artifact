#ifndef KERNEL_LIKELIHOOD_H_
#define KERNEL_LIKELIHOOD_H_
    ...
    const int max_size)
{
  ...
  if(i < Nparticles)
  {
    for(y = 0; y < countOnes; y++){
      ...
      ind[i*countOnes + y] = abs(videoSequence->get_index(indX, indY, k));
      if(ind[i*countOnes + y] >= max_size)
        ind[i*countOnes + y] = 0;
    }
    float likelihoodSum = 0.0f;
    VideoSequence &I = *videoSequence;
